...
<button command="show-modal" commandfor="{site.title}-dialog" class="p-2 hover:bg-hover rounded-lg flex flex-col justify-between cursor-pointer border-2 border-text font-semibold {classes}">
...
{#snippet topbar()}
    <h3 id="dialog-title" class="font-semibold text-xl">{site.title}</h3>
    <button type="button" command="close" commandfor="{site.title}-dialog" class="flex flex-col content-center justify-center rounded-md text-sm font-semibold border border-text hover:bg-hover cursor-pointer ml-3 p-3 w-auto">Close</button>
{/snippet}
